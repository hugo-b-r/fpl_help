...
use eframe::egui;
use fpl_help::{convert_coordinates, get_coordinates, url_from};
use geocoding::Point;
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if text != *"" {
            egui::TopBottomPanel::bottom("trip_coordinates").show(ctx, |ui| {
                ui.add_sized(
                    [egui::Ui::available_width(ui), 20.0],
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            for point in coordinates.iter() {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::RIGHT), |ui| {
                        #[cfg(not(target_arch = "wasm32"))] //not copying or pasting on web
                        ...
                        if ui.button("Add").clicked() {
                            flight_plan_coordinates.push(format!(
                                "{} {}",
                                convert_coordinates(*point).unwrap(),
                                address
                            ));
                            println!(
                                "Add {} to list of destinations",
                                format!("{} {}", convert_coordinates(*point).unwrap(), address)
                            );
                        }
            ...
            if !flight_plan_coordinates.clone().is_empty() {
                // if no coordinates, we don't show
                ui.add_space(12.0);
                ui.separator();
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::RIGHT), |ui| {
                            if ui.button("Remove").clicked() {
                                let index = flight_plan_coordinates
                                    .clone()
                                    .iter()
                                    .position(|x| *x == *coordinates)
                                    .unwrap();
                                flight_plan_coordinates.remove(index);
                            }
                            if ui.button("Copy coordinates").clicked() {
                                let _ =
                                    &clipboard.set_text((*coordinates)[0..11].to_string().clone());
                            }
